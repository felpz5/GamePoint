...
class GameCafeApp(ctk.CTk):
    ...
    def configurar_aba_gestao_mesas(self):
        ...
        botao_atualizar = ctk.CTkButton(controles_frame, text="🔄 Atualizar", command=self.carregar_mesas, 
    ...
    def carregar_mesas(self):
        ...
        for i, mesa in enumerate(mesas):
            ...
            if status == "ocupada":
                if mesa["start_time"]:
                    ...
                    if tempo_restante_segundos > 0:
                        ...
                        if tempo_restante_segundos < 600: # Alerta vermelho 10 min
                             cor = ACCENT_RED
                    else:
                        texto_status += "\nTEMPO ESGOTADO"
                        cor = ACCENT_RED
            ...
            frame_mesa = ctk.CTkFrame(row_frame, fg_color=cor, corner_radius=15, width=200, height=150)
            ...
            ctk.CTkLabel(content, text=f"Mesa {mesa['id']}", font=("Arial", 20, "bold"), text_color="white").pack()
            ctk.CTkLabel(content, text=mesa["console"], font=("Arial", 14), text_color=GOLDEN_YELLOW).pack()
            ctk.CTkLabel(content, text=texto_status, font=("Arial", 16), text_color="white").pack(pady=5)
            
            if status == "ocupada":
                btn_liberar = ctk.CTkButton(content, text="Liberar", command=lambda m=mesa: self.liberar_mesa(m), 
                                            fg_color="white", text_color=cor, width=100, height=30, corner_radius=8)
                btn_liberar.pack(pady=5)
            elif status == "livre":
                btn_ocupar = ctk.CTkButton(content, text="Ocupar", command=lambda m=mesa: self.abrir_gerador_codigo(m), 
                                            fg_color="white", text_color=cor, width=100, height=30, corner_radius=8)
                btn_ocupar.pack(pady=5)
                
        # Garante que a atualização do tempo continue se estiver na aba
        if self.tabview_func.get() == " 🖥️ Gestão de Mesas ":
            self.employee_frame.after(1000, self.carregar_mesas)
    ...
    def carregar_pedidos(self):
        ...
        # Garante que a atualização continue se estiver na aba
        if self.tabview_func.get() == " 📦 Pedidos ":
            self.employee_frame.after(5000, self.carregar_pedidos) # Atualiza a cada 5 segundos
    ...
    def ocupar_mesa_com_codigo(self, mesa_id_str, tempo_minutos_str, nome_cliente):
        try:
            ...
            self.carregar_mesas() # Atualiza a aba de gestão de mesas
    ...
    def liberar_mesa(self, mesa):
        if messagebox.askyesno("Confirmar Liberação", f"Confirma a liberação da Mesa {mesa['id']}? Isso finalizará o tempo e o código de acesso."):
            try:
                ...
                self.carregar_mesas() # Atualiza a UI
